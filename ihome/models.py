...
class User(BaseModel, db.Model):
    ...
    def user_dict(self):
        u_dict = {
            ...
            "avatar": constants.QI_NIU_DOMAIN + self.avatar_url if self.avatar_url else ""
        }
        return u_dict
...
class House(BaseModel, db.Model):
    ...
    #获取房屋的基本展示信息
    def get_house_basicInfo(self):
        basic_dict = {
            "house_id":self.id,
            "title":self.title,
            "price":self.price,
            "area_name":self.area.name,
            "image_url":constants.QI_NIU_DOMAIN + self.index_image_url if self.index_image_url else "",
